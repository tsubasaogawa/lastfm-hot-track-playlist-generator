...
func main() {
	var (
		...
		duplicate = flag.Bool("duplicate", false, "Allow duplicate playlists with same title")
		dryrun    = flag.Bool("dryrun", false, "Do not generate a playlist but fetch weekly chart")
	)
	flag.Parse()
	...
	tracks, err := lastfm.GetTracks(*user, os.Getenv("LASTFM_API_KEY"), str2unixtime(*from), str2unixtime(*to))
	if err != nil {
		log.Fatalln(err)
	}
	if *dryrun {
	...
	playlist, err := ytmusic.NewPlaylist(service, *title, *desc, *privacy, *duplicate)
	if err != nil {
		log.Fatalln(err)
	}
